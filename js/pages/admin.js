...
router.put('/settings', wrap(async (req, res) => {
  ...
  await saveSettings({ referral_reward, min_withdraw, max_withdraw, welcome_text });
...
router.get('/withdrawals', wrap(async (req, res) => {
  ...
  const { rows } = await pool.query(
    `SELECT w.id, w.amount, w.address, w.status, w.created_at AS date,
            u.id AS user_id, u.first_name, u.last_name, u.username
...
// ---------- Users ----------
const USER_SELECT = `
  SELECT u.id, u.first_name, u.last_name, u.username, u.balance, u.created_at,
